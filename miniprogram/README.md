...
原生微信小程序（扫码出库、隐患登记与整改跟进、台账查看），使用 TDesign Mini Program 组件库。启动后通过 `wx.login` 无感登录；未注册用户只有在提交姓名后才会创建 OpenID 与姓名映射，管理端不支持手工新增。之后可扫描物资小程序码直接进入对应物资的出库页面。

## 功能开关

各功能页的模式由网页端「高级设置」下发（`utils/features.js` 的 `xxx_mode`）：`disabled` 隐藏入口、`query_only` 只读、`read_write` 可写（库存可出库、隐患可登记与跟进）。台账查看是纯浏览功能（列表 + 搜索 + 详情），只提供 `disabled` / `query_only` 两档。开关只做前端入口拦截，后端数据接口不按它鉴权。
...
storage 的 `miniProgramThemeMode`，不落库、不产生请求。首页「个人信息」弹窗里的「外观」一行可切换三档：点击后弹出 `t-picker` 滚轮选择器，默认停在当前档位，确认才生效（取消不改动）。
...
列表页转发给同事时，会把当前的搜索词与筛选值一起写进分享链接：对方点开分享卡片，**已注册就直接看到同一份筛选结果**；未注册则先绑定，绑定后由回跳参数自动落到同一份结果。覆盖申购计划、申购记录、隐患管理、二级库库存、物料编码、华星总库存与台账查看。

编解码集中在 `utils/share.js`（小程序 `onLoad` 的 query 不会自动解码，读取时统一解码）；分享值已不在筛选候选项里时会补进下拉，固定枚举（库存状态、隐患状态）则校验后再采用。
