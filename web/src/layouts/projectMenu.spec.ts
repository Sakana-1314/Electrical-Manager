...
const projects = [project(), project({ id: 2, name: '二期项目', is_default: false })]

describe('projectMenu（用户菜单的项目二级菜单）', () => {
  it('父项显示当前项目名称，子项只显示名称', () => {
    ...
    expect(parent.label).toBe('项目：华星现有项目')
    ...
    // 子项只写名称
    expect(children.map((child) => child.label)).toEqual(['华星现有项目', '二期项目'])
  ...
  it('子项标签只显示名称（没有编码这回事）', () => {
    expect(projectOptionLabel({ name: '华星镍业二期' })).toBe('华星镍业二期')
    expect(projectOptionLabel({ name: ' 华星现有项目 ' })).toBe('华星现有项目')
    // 名称意外为空时退回占位文案，菜单项不会变成空白
    expect(projectOptionLabel({ name: '  ' })).toBe('未命名项目')
  })
