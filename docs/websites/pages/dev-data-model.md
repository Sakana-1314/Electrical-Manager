...
| 后台可见功能 | `WebFeatureVisibility` | 管理端侧栏主 tab 的可见性配置（工作台 / 备忘录 / 二级库 / 华星总库存 / 申购管理 / 隐患管理 / 台账管理 / 工作管理，每项「显示 / 隐藏」）：落在 `system_setting` 的 `ai_search_config.web_features`（JSON），只影响侧栏渲染与落地页推导，不拦路由与接口；系统管理固定显示，不参与该配置 |
...
| 台账标签 | `ledger_tag` / `ledger.tag_ids` | 标签是自引用邻接表（`parent_id`，一级标签为 NULL），至多 3 层，同一层级下名称唯一；标签可改上级（PATCH 传 `parent_id`，`null` 即移为一级标签），改后整棵子树重新计入层级，超过 3 层或移入自身子孙下会被拒绝。台账记录的标签以英文逗号分隔的标签 id 存在 `ledger.tag_ids`（空串 = 未挂标签），读接口按 id 回填名称与完整层级路径。孤立标签 = 既无父节点又无子节点，树标签 = 有父或有子；按标签筛选时命中选中标签及其全部子孙标签 |
