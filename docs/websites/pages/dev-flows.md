...
flowchart TD
    ...
    B --> B1["待删除（软删除、保留期内）同样可读：附件管理页要展示预览图才能判断是否撤销删除；物理清除后记录已不存在，返回 NOT_FOUND"]
    D["删除图片"] --> E{"被任一业务关联表引用？"}
    E -- "是" --> F["拒绝：文件正在被使用"]
    E -- "否" --> G["软删除：只落 deleted_at，立即从业务隐藏，回执给出 purge_after"]
    G --> G1["保留期 7 个完整自然日（期间可撤销删除）"]
    G1 --> G2["到期后第一个凌晨 2 点：复查引用"]
    G2 -- "仍无引用" --> G3["物理清除数据库行与磁盘文件"]
    G2 -- "又被引用" --> G4["撤销删除，恢复在用"]
...
网页端 `ImageUploader.vue` 的并发只在前端排队：多选（或粘贴）后进入本地队列，最多 2 个请求在途、其余排队，每个文件一行独立进度，失败项只影响自己并可重试；上传在途时添加按钮仍可点，可继续选图或粘贴，新文件进同一队列排队。逐项进度最多显示到 99%（请求体发完不等于服务端解码、写盘、落库完成），真正完成时该项直接从队列消失。进度下方同时显示**用时**（每秒刷新），从排到自己真正开始时起算、排队不计入，失败项冻结本次耗时以便回看。删除已上传图片先弹二次确认，确认后才调删除接口。

上传组件通过 `v-model:busy` 把「队列里还有在途项（含排队）」抛给父级，父级据此禁用保存按钮，避免提交时 `image_ids` 漏掉还没传完的图；已失败项是终态、不阻塞保存，用户可自行重试或移除。同一表单有多个上传组件（如隐患的整改前 / 后）时各绑一个 `ref`，保存按钮按「任一为真即禁用」判断。
