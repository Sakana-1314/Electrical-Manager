...
网页端 `ImageUploader.vue` 的并发只在前端排队：多选（或粘贴）后进入本地队列，最多 2 个请求在途、其余排队，每个文件一行独立进度，失败项只影响自己并可重试；上传在途时添加按钮仍可点，可继续选图或粘贴，新文件进同一队列排队。逐项进度最多显示到 99%（请求体发完不等于服务端解码、写盘、落库完成），真正完成时该项直接从队列消失。删除已上传图片先弹二次确认，确认后才调删除接口。
