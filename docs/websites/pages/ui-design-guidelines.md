...
存储上是「一行一个组合」的字典（如隐患类型的 `大类 + 小类`）用该组件呈现为两级树：**大类在左、小类在同一水平带上向右展开**，父子连线由组件自带的 CSS 伪元素绘制，页面**不自绘 SVG 或边框**。

| 项 | 要求 |
| --- | --- |
| 触发条件 | 仅当数据确实是两级（父 + 子）且层级要在页面上可见时使用；纵向缩进场景用 `n-tree`，不要用它 |
| 数据 | 传**新建对象**（如 `buildHazardTypeTree()` 的产物），不要把接口列表直接传入——组件会往节点写 `$` 前缀字段 |
...
| 操作范围 | 只维护**叶子**（业务实体行）；分组不是实体，不提供整组改名/删除 |
| 样式 | 组件自带布局面向「可拖拽组织图」（外框白底、内容层绝对定位、内层 `height:100%`），放进卡片会高度塌陷且深色档露白底。`web/src/styles.css` 的「隐患类型横向树」段已统一还原为文档流并映射 `--color-*` 令牌；新页面复用同一段，不要另写一套 |
