...
| 控件字号 | 输入框 / 下拉 / 日期 / 数字输入的**文字与占位文字**、按钮、分页、表单标签统一走 `theme.ts` 的 `fontSizeMedium`（`13px`）；表格、描述列表、标签在各自组件覆盖里显式保持 `14px`——数据展示不跟着控件一起缩小。次要说明文字（`.muted`，如「最后更新：…」）同为 `13px` |
...
| 下拉内容 | 先「外观」二级菜单（悬浮向左展开自动 / 浅色 / 深色，当前档勾选），分隔线之后是「退出登录」 |
